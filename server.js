...
async function getnewToken(url,refresh_token)
{
  const newAccess = await fetch(url,{
    method: 'POST',
    headers: {
      'Content-Type' : 'application/x-www-form-urlencoded',  
    },
...
async function addToPlaylist(playlistID,refresh_token,uris)
{
  ...
  const result = await getnewToken(spotify_token_url,refresh_token);

  const output = await fetch(`https://api.spotify.com/v1/playlists/${playlistID}/tracks`,{
    method: 'POST',
...
const token =  await _getToken(youtube_token_url,req.query.code,process.env.YoutuberedirectUrl,process.env.YoutubeclientId,process.env.YoutubeclientSecret)

const resp = await fetch('https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&myRating=like&maxResults=50',{
     method: 'GET',
...
app.get('/getSpotifyUris/',function(req, res)
{
   ...
   (async function getURIs()
   {
    ...
    const uriPromise = LikedDetails.map(async (details) => {
      ...
      const search = await fetch(`https://api.spotify.com/v1/search?q=track:${details.song}%20artist:${details.artist}&type=track`,{
      method: 'GET',
    ...
    res.redirect(`https://accounts.spotify.com/authorize?client_id=${process.env.clientId}&response_type=${response_type}&redirect_uri=${process.env.spotifyAuth}&scope=user-read-private%20playlist-modify-private%20user-read-email%20playlist-modify-public&state=34fFs29kd09`);
   })()
...
const token =  await _getToken(spotify_token_url,req.query.code,process.env.spotifyAuth,process.env.clientId,process.env.clientSecret);
...
const checkPlaylist = async () =>
{
    ...
    const result = await getnewToken(spotify_token_url,input.refresh_token);
...
return {Exists: playlistExists, refresh_token: input.refresh_token, id:input.id, PlaylistID: Playlistid}
}
...
if (input.Exists === false)
{
 ...
 const playlist = await fetch(`https://api.spotify.com/v1/users/${input.id}/playlists`,{
  method: 'POST',
  headers: { 'Authorization' : `Bearer ${result.access_token}`
 },
...
const Newplaylist = await playlist.json();
  ...
  const result2 = await getnewToken(spotify_token_url,input.refresh_token);

  fetch(`https://api.spotify.com/v1/playlists/${Newplaylist.id}/tracks`,{
  method: 'POST',
  headers: { 'Authorization' : `Bearer ${result2.access_token}`,
...
else
{
  ...
  const playlist = await fetch(`https://api.spotify.com/v1/playlists/${input.PlaylistID}/tracks`,{
   method: 'GET',
   headers: { 'Authorization' : `Bearer ${result.access_token}`
  },
 ...
 {
  const output = await addToPlaylist(input.PlaylistID,input.refresh_token,tobeAdded);
  console.log(output);
 }
